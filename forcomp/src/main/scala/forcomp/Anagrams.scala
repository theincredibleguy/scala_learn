...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = ((w.toLowerCase map (c => (c, 1)) groupBy (_._1) map (e => (e._1, e._2.size))) toList) sorted
...
//    dictionary map (word => (wordOccurrences(word), word)) groupBy(e => e._1) map(f => (f._1, f._2.unzip._2))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val perm: List[Occurrences] = occurrences map (o => (for {x <- 1 to o._2} yield (o._1, x)).toList)
    perm.foldRight(List[Occurrences](Nil))((o, y) => y ++ (for { x <- o; j <- y } yield ((x :: j))))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    (for {
      (c, i) <- x
      val t = yMap getOrElse(c, 0)
      if (t != i)
    } yield (c, if (t > i) i else (i - t))) sorted
  }
